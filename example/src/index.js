...
function getThirdwebContract(
	...
	const tokenContract = getThirdwebContract(tokenAddress, client);
	console.log('[getThirdwebContract]: tokenContract = ' + JSON.stringify(tokenContract));
	// 2. Get airdrop contract
	const airdropContract = getThirdwebContract(airdropAddress,client);
	console.log('[getThirdwebContract]: airdropContract = ' + JSON.stringify(airdropContract));
	return {
...
const THIRD_WEB_CLIENT_SECRETE = 'tGzGcXEEIW9ooVydHd79JUfStwJ8BMnoyGKcD5tpV1g1Kn-ypAcOX4ulbn-dV4F7QZXttffAEZabanAHjJp83g'
...
const {
	airdropContract
} = getThirdwebContract(THIRD_WEB_CLIENT_SECRETE, TOKEN_CONTRACT_ADDRESS, AIRDROP_CONTRACT_ADDRESS);
